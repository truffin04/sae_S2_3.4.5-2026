...
def client_liste_envies_show():
    mycursor = get_db().cursor()
    id_client = session['id_user']
    chaussures_liste_envies = []
    chaussures_historique = []
    return render_template('client/liste_envies/liste_envies_show.html'
                           ,chaussures_liste_envies=chaussures_liste_envies
                           , chaussures_historique=chaussures_historique
                           #, nb_liste_envies= nb_liste_envies
                           )



def client_historique_add(chaussure_id, client_id):
    mycursor = get_db().cursor()
    client_id = session['id_user']
    # rechercher si l'chaussure pour cet utilisateur est dans l'historique
    # si oui mettre
    sql ='''   '''
    mycursor.execute(sql, (chaussure_id, client_id))
...
def client_liste_envies_chaussure_move():
    mycursor = get_db().cursor()
    id_client = session['id_user']
    id_chaussure = request.args.get('id_chaussure')
